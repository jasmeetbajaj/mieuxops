...
export const Sidebar = () => {
  ...
  return (
    <div className="w-64 bg-sidebar text-sidebar-foreground h-screen flex flex-col fixed left-0 top-0 border-r border-border/10 shadow-2xl">
      <div className="h-16 flex items-center px-6 font-bold text-xl tracking-tight border-b border-border/10">
        <span className="text-primary mr-1">Mieux</span>Flow
      </div>
      ...
      <div className="p-4 border-t border-border/10">
        ...
        <button 
          onClick={logout}
          className="w-full flex items-center space-x-2 px-3 py-2 rounded-lg text-sm font-medium text-destructive/80 hover:bg-destructive/10 hover:text-destructive transition-colors"
        >
          <LogOut size={18} />
          <span>Logout</span>
        </button>
